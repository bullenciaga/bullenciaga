...
They are Custom Domains in `wrangler.production.jsonc`, attached to the existing
...
version, verifies the primary site, applies configured domain bindings with
`wrangler triggers deploy`, and checks every current short link through both
