(function () {
  ...
  function render(payload) {
    ...
    document.getElementById('passport-title').textContent = 'Wallet record';
    ...
    notice.textContent = `Read-only record refreshed ${new Date(payload.updatedAt).toLocaleString()}.`;
  ...
  async function load(wallet) {
    ...
    notice.classList.remove('is-error');
    try {
      const response = await fetch(`/rpc/passport?wallet=${encodeURIComponent(wallet)}`, { headers: { accept: 'application/json' } });
