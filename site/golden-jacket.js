...
export function installGoldenJacket(authority){
  ...
  const final=Array.isArray(campaign.finalResults)&&campaign.finalResults.length===5;
  ...
  async function refresh(){
    try{
      ...
      const group=final?'completed':s.phase==='not-started'?'upcoming':s.phase==='drawn'?'completed':s.counting?'active':'pending';
      article.dataset.status=group;article.querySelector('.status').textContent=final?'Completed':health.behind?'Catching up · Buy competition':s.counting?'Live · Buy competition':s.phase==='drawn'?'Winners selected':s.phase.replaceAll('-',' ');
      ...
      if(s.counting&&!final){const a=node(actions,'a','Buy $BULLEN','action primary');a.href='/buy';}
      if(s.amendment&&!final){
      ...
      }else if(!final)node(root,'p',s.closesAt?'Entry deadline: '+new Date(s.closesAt).toLocaleString(): 'Counting has not started.');
      if(s.problem&&!final)node(root,'p',s.problem);
      if(final){
        node(evidence,'p','The original automated selection and sealed snapshot remain available as campaign records.');
        const record=node(evidence,'a','View the original selection record');record.href='/competition-api/status';
      }
      if(s.updatedAt&&!final)node(evidence,'p','Last verified: '+new Date(s.updatedAt).toLocaleString()+(s.phase==='drawn'?'.':'. Rankings are provisional until the draw snapshot is sealed.'));
      ...
      if(!final){if(ranked.length){node(evidence,'h4','Qualifying net purchases');const list=node(evidence,'ol','');for(const r of ranked)node(list,'li',r.wallet.slice(0,5)+'…'+r.wallet.slice(-5)+' · '+Number(BigInt(r.countedRaw)/1000000n).toLocaleString()+' $BULLEN');}else node(evidence,'p','No qualifying purchase entries yet.');}
      if(s.winners&&!final){const result=node(root,'div','');node(result,'strong','Winners · Manual delivery pending');const list=node(result,'ul','');for(const w of s.winners)node(list,'li',`HERD #${w.position}: ${w.wallet||w.reason}`);}
    ...
    }catch{if(final){article.dataset.status='completed';article.querySelector('.status').textContent='Completed';root.replaceChildren();evidence.replaceChildren();node(evidence,'p','The original selection record is temporarily unavailable.');}else{article.dataset.status='pending';article.querySelector('.status').textContent='Status unavailable';const panel=root.querySelector('.golden-volume');if(panel){let notice=root.querySelector('.verification-unavailable');if(!notice)notice=node(root,'p','','verification-unavailable');notice.textContent='Verification is temporarily unavailable. The total above is the last verified count.';}else{root.replaceChildren();node(root,'strong','Live progress is temporarily unavailable. Please check again shortly.');}}document.getElementById('activeCount').textContent=document.querySelectorAll('.campaign[data-status="active"]').length;article.querySelector('.actions')?.replaceChildren();}
  }
